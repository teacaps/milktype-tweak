...
function buildDefinitions() {
  if (fs.existsSync('./public/definitions/v2')) {
    fs.rmSync('./public/definitions/v2', {recursive: true});
  }
  if (fs.existsSync('./public/definitions/v3')) {
    fs.rmSync('./public/definitions/v3', {recursive: true});
  }
  ...
  for (const version of ['v2', 'v3']) {
    ...
    for (const filename of fs.readdirSync(`./public/keyboards/${version}`)) {
      ...
      const keyboardDefinition = require(
        `../public/keyboards/${version}/${filename}`,
      );
      const viaDefinition =
        version === 'v2'
      ...
      fs.writeFileSync(
        `./public/definitions/${version}/${filename}`,
        JSON.stringify(viaDefinition, null, 2),
      );
